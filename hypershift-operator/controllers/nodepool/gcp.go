...
func configureGCPNetworkTags(userTags []string, infraID string) []string {
	...
	if infraID != "" {
		tags = append(tags, toGCPNetworkTag(fmt.Sprintf("%s-worker", infraID)))
	}
...
// toGCPNetworkTag converts a tag to GCP-compliant network tag format.
// Network tags must match: (?:[a-z](?:[-a-z0-9]{0,61}[a-z0-9])?)
// - Must start with a lowercase letter
// - Can only contain lowercase letters, numbers, and hyphens
// - Maximum 63 characters
func toGCPNetworkTag(tag string) string {
	...
	// Ensure it starts with a lowercase letter - prefix with 'h' (for hypershift) if needed
	if len(result) > 0 && (result[0] < 'a' || result[0] > 'z') {
		result = "h-" + result
	}

	// Ensure it ends with a letter or number (remove trailing hyphens)
	result = strings.TrimRight(result, "-")

	// Truncate to 63 characters max to meet GCP requirements
	if len(result) > 63 {
		result = result[:63]
		// Ensure we didn't end with a hyphen after truncation
		result = strings.TrimRight(result, "-")
	}
